...
| Finance / Payroll Manager | Payroll, compensation, finance categories, expenses, budgets | Payroll setup/runs, finance categories, expenses, budgets | Payroll and expense approval/payment | Payroll and finance reports | Should not approve activity/project workflows unless separately allowed. |
...
| Auditor | Tenant oversight and read-only reports | None | None | Reports, payroll runs/payslips, finance/expense views, audit logs | Must remain read-only; cannot create/edit/approve/pay/export payroll CSV unless explicitly allowed. |
